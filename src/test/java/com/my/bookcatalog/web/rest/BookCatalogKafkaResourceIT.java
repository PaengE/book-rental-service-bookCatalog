...
class BookCatalogKafkaResourceIT {
    ...
    void setup() {
        ...
        BookCatalogKafkaResource kafkaResource = new BookCatalogKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/book-catalog-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/book-catalog-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
